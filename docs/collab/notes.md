...
[2026-07-28 TESTER] ossfs2 顺序写 EINVAL 修复（段1 `--parallel 1 --part-size 5Gi` + failure_detail + poll_once error_detail + result_card 明细 + max_polls 48h→7天）补测完成。
  新文件 `tests/unit/test_ssh_transfer_ossfs2_einval.py`（35 例）：① **回归钉死**（5 例）start_stage1 命令必须含独立 token `--parallel 1`（且值不得 >1）与 `--part-size 5Gi`（取自模块常量 `_OSSFS2_PART_SIZE`，两处不许漂移）、`--jobs`(复数) 恒不出现且 `--job 30`(单数,跨文件并发) 保留、三 flag + `-u`/`--checkpoint-dir` 都落在 `ossutil cp` 那段；文件头写清真机取证（≤100MiB 33484 全成功 / >100MiB 42366 全失败、删 flag 就 100% 失败）。做了**变异验证**：修复前的命令串对 `--parallel/--part-size` 两条正则均为 False、`--parallel 4` 被「不得 >1」那条抓住 → 断言非空。② failure_detail（12 例）汇总行+报告路径+`失败对象 N 个`+`首条根因` / 无 .report 只回汇总行且不发第二条命令 / `invalid argument` 追加 >5Gi 说明（首条命令抓到 + 只在 report cause 里抓到 两态 + 大小写变体）/ 第一条 run 抛异常→返 "" 不外抛、第二条抛异常→best-effort 保留已拿到部分 / COUNT=0 不吹「失败对象 0 个」/ 长度恰 `_DETAIL_MAX=1200` / **命令含 `tr '
' '
'` 且顺序 tail→tr→grep**（不 tr 只能抓到进度条，本次排障关键教训）/ grep 含 FinishWithError·rsync error 等标记 + 定位到对应 stage 的 log marker / 无匹配行返 ""。③ poll_once（5 例）FAILED 填 error_detail 并落库、段号对得上（stage1/stage2 各摘自己的日志）、**failure_detail 抛异常仍正常落 FAILED + error_detail=="" + finished_ts**（取明细失败不带崩终态判定）、DONE/RUNNING 不取明细（不白烧 SSH）。④ result_card（5 例）有明细→「明细」块逐行 `· ` 前缀 + 重试按钮仍在 + 仍 1.0；无明细→元素数少 1、tags==[div,div,hr,action] 与改动前一致；空/空白/None 明细当无（参数化 5）；明细空行跳过；DONE 不显示明细。⑤ run_to_completion（2 例）`max_polls` 默认 10080 且 `default*poll_interval>=7天`、显式断言 `!=2880`（防手滑调回 48h）；超时分支仍可达。⑥ **端到端串联**（2 例）poll_once 摘到的明细一路进 result_card；按 ID 查失败的 sgp- 任务→补推卡带明细（文本回执仍只带 error 摘要，记录现状）。
  **测试卫生加固**（tests/conftest.py）：新增 autouse `_no_real_ssh`，把 `engine_ssh._client` 打成硬失败。本机没 paramiko 时无感，但**服务器容器 aiops-bot 装了 paramiko + .env 有真私钥**，那里跑单测时 poll_once 的 FAILED 分支会调 failure_detail→run→真连 SGP 生产机读远端日志。同 #57 的飞书误发教训，fail-closed 掉。
  全量默认 pytest **1539 passed / 1 skipped(h5py 缺,既存) / 9 deselected(integration) / 0 failed**（本地 .venv py3.11，跑两遍：随机序 + `-p no:randomly` 均稳定；1504 基线 + 35 新例）。**无既存失败**（matplotlib 两例本地绿，容器缺 matplotlib 才 red，属环境基线）。**未发现 dev 改动里的真问题**；3 条 LOW 已发 dev（明细截断把最有用的 >5Gi 说明排在最后最先被截 / `tail -c 4000` 若尾部输出 >4KB 可能漏掉 report 路径 / 换 part-size 后旧 ckpt1 与残留 `.temp` 的清理）。**E2E 归 dev**：flags 已有 dev 真机铁证（单文件 rc=0/362198872B + 200 对象 77.6GiB 148MiB/s 零 EINVAL），failure_detail/明细卡需部署后真跑一单看卡；我这边 ssh 到服务器被权限门挡住，未在容器复跑。

[2026-07-28 TESTER] 上条的 delta 收口（dev 按 auditor LOW-3 + 我的 LOW-1/2 改了逐行截断/20KB 尾窗/`Error:`/report 白名单/超时/先落库再摘明细/清 error_detail/CLI 明细）：同文件 35→**56 例**，全绿。
  改 1 例：`test_failure_detail_truncated_to_detail_max` 精确 `== 1200` → **`<= _DETAIL_MAX`**（逐行 `_clip` 后 4×500 字符只剩 963，真实不变量是「≤上限」）。
  新增 20 例：① **长汇总行不得挤没根因**（本次改动真实意图，`test_..._long_summaries_do_not_squeeze_out_root_cause`：3 条 500 字符汇总行 + report 命中 + EINVAL → `失败对象`/`首条根因`/`ossfs2` 三者都还在）+ 单行截到 `_DETAIL_LINE_MAX==240`；② 探针硬化：`tail -c 20000` + `tail -n +2`（顺序也断言）、远端兜底分支（`if [ -n "$f" ]` / `grep -av '^[[:space:]]*$'` / `tail -3` 恰两处）、两条命令超时 `[20,25]`、grep pattern 含 `Error:`；③ 裸 `Error: ... AccessDenied`（前一单 sgp-841b88a7b0dd rc=2 异地桶形态）能完整渲染进明细且不发第二条命令；④ **report 路径白名单对抗**（参数化 6：`$(id)`/`;rm`/空格/反引号/`.report_evil`/相对路径 → 一律不拿去 grep，防被日志里构造的对象 key 骗成任意文件读取 oracle）+ 真机形态含 `.`/`-`/`_`/多级目录正常采用；⑤ 段2 的 EINVAL **不套**段1 ossfs2 结论（改说泰国侧目标目录/挂载点）；⑥ poll_once **先落库再摘明细**（在 failure_detail 里回读 Redis 断言那时已是 FAILED，避免对账 stale 门白跑一遍远端 grep）；⑦ `_start_stage` 起任务即挂 → 清掉上一轮 `error_detail` 并落库（不让「起 stage1 失败」配上上一轮「42366 个 + ossfs2 说明」自相矛盾）；⑧ `_h_retry_ssh_transfer` 同样清 `error_detail`、`stage1_rc` 仍保留；⑨ 超时文案 ≥48h 说「2天」不说「48h」、<48h 仍用「2h」（桩 `orch.time.sleep`）；⑩ CLI `status` 有明细逐行打印 / 无明细不打空段。
  **变异验证**（断言非空）：旧写法「先 join 再 `[:1200]`」下同一输入 → `失败对象`/`首条根因`/`ossfs2` **三者全为 False**（会 FAIL），逐行 clip 后三者全 True、总长 863。
  子集 `-k ssh_transfer` **278 passed**；全量默认 pytest **1560 passed / 1 skipped(h5py 既存) / 9 deselected(integration) / 0 failed**（跑两遍：随机序 + `-p no:randomly` 一致）。既存失败 0。未发现新源码问题；上轮那 3 条 LOW 已被 dev 全部落实。

[2026-07-29 AUDITOR] #62 增量复审（_safe_report_path 白名单 / 远端兜底 / 清 error_detail / 先落库 + 6 低）：**可提交无阻塞**。白名单两处 `search` 绕过（`/root/x.report_evil`、`relative/x.report`）已封、`..` token 级粒度正确、远端 shell 引用/空值/多行经本地 bash 实测安全。1 中：`tail -n +2` **无条件**丢首行 → 单行日志（异地桶 rc=2 那种裸 `Error:` 快速失败）明细被吞成空，dev 已改为 `wc -c > 20000` 才丢（单测桩掉 run 抓不到，属远端 shell 语义）。4 低：.report oracle 仅收窄未封死、真机日志行标点会让 report 段静默失效、`run_to_completion` 超时分支未清 `error_detail`、`_DETAIL_MAX` 预算可能削掉 ossfs2 说明 —— 均已落实。
  **dev 记：目录白名单（auditor 可选低危项）刻意未采纳。** 加「report 必须在 job 目录或 /ossutil_output/ 下」后，ossutil 报告目录一变，「首条根因」会静默消失 —— 而那正是本函数存在的意义。注入面已由结构白名单封死；残留泄漏面需攻击者先能写源桶对象名、再猜中真实存在的 .report 路径、且仅泄漏一行。权衡取诊断能力，理由已写进 `_safe_report_path` docstring。

[2026-07-29 DEV] #62 真机定因（比单测更硬的证据链）：
  ① **失败边界与分片阈值严格吻合**：源 75850 对象 / 19.527 TiB 中，≤100MiB 的 **33484** 个全成功、>100MiB 的 **42366** 个全失败，与 ossutil 报告 done/failed 计数一字不差；报告 42367 行里 `invalid argument` 出现 42366 次、其它 Error 0 次。
  ② **单文件对照**：同一 345.4MiB 对象、同一挂载点 —— 现行 flags rc=4；`--parallel 1 --part-size 5Gi` rc=0、362198872 B 完整落地。高并发复验 `--job 30` 跑 200 对象/77.6GiB 零 EINVAL、**148MiB/s**（比出错那次的 75MiB/s 快一倍），故不降并发。源最大对象仅 0.488GiB，5Gi 上限可全覆盖。
...
[2026-07-29 TESTER] #63 第二阿里云主账号（账号档案注册制）补测收口。**本机 .venv 全量 1738 passed / 1 skipped(h5py 既存) / 1 xfailed(见下方真 bug) / 9 deselected(integration) / 0 failed**（跑多遍：默认序 + `-p no:randomly` 全一致）；**服务器容器 aiops-bot（已部署 2808296，.env 真配了 1949）全量 1734 passed / 2 failed / 3 skipped / 1 xfailed / 9 deselected** —— 2 failed 是 `test_gpu_distribution::test_dist_url_and_summary_card` + `test_wants_metrics_chart::test_process_message_attaches_chart_for_metrics`，容器无 matplotlib 模块，**环境基线、任何 commit 都复现**（已核 `import matplotlib` → ModuleNotFoundError）；3 skipped = h5py + 2 条 pypinyin 降级分支（容器装了 pypinyin 故跳，本机未装故跑）。temp_ak 子集：本机 447 / 容器 444+3skip。
  **新增 2 文件 177 例**（容器内亦 177 passed）：
  · `tests/unit/test_temp_ak_accounts.py`(74)：档案装配（三项全空才不注册 / 任一存在即注册的三参数化 / 注册但缺 AK → 前缀可用但 ram_client 抛 UnknownAccountError / 缺 code → 不进 issue_codes 不被路由 / profiles() 不缓存）、settings 11 新字段登记、1949 五个 widget id + 无 platform 键、frozen 档案不可就地篡改、by_issue_code（互斥/空串不匹配空 code 档/trim）、by_grant_id（tak- 与 tak1949- 不互认 + 最长前缀优先 + 未注册返 None + 拒 xpfs-/sgp-/TAK- 等）、by_slug（未注册**抛错**不退默认档）、ram_client 显式传该档 ak/sk、issue_codes、bucket_map 按档+坏 JSON、**默认档零回归**（前缀/命名/显示名/`_KEY_PREFIX`/`_LOCK_PREFIX` 逐字对齐 + resolve_bucket 仍回退 permsync.BUCKET_MAP + `permsync_client()` 零参路径三态）。
  · `tests/unit/test_temp_ak_account_isolation.py`(103)：I1 命名空间（两档 grant 各落各前缀、反向不出现、同 instance_code 出两条独立记录、user/policy 名带账号前缀、claim 锁分离）；I2 sweep 逐档（各自 client 删各自用户 + **默认 client 绝不碰 1949 用户** + 扫描键恰 `["temp_ak:grant:*","temp_ak_1949:grant:*"]` + 只抹 AK 仍扫得到但删不成 + 整档下线不扫不误删 + 某档挂不阻塞另一档）；I3 revoke 按账号取 AK（抹 AK / 整档下线 两态均 fail-closed 且零 client）；I4 共用延长/撤销按 `tak1949-`/`tak-` 分派（含两档 grant 同 hash 时不误动 + 撤销分支 + 防串）；I5 桶映射按账号（同名展示桶→不同真实桶、1949 不回退 permsync.BUCKET_MAP、1949 表不泄漏进默认档）；I6 **命门** `make_ram_client(ak,sk)` 显式传参在 env `ALIBABA_CLOUD_*` 已设时仍用传入那对（+ 半参**抛错**不回落 + 零参旧行为 env→settings 逐字不变 + accounts.ram_client / _issue_ram 端到端）；**STS 跨账号越权专项**（classify_mode 非默认档恒 RAM 即便阈值 43200 / `_issue_sts` 硬门抛 IssueError 且 STS 桩零调用 / issue() 分发同样拦 / 默认档 STS 零回归 / plan 不给 1949 出 STS 计划 / 延期 STS→RAM 转换用 1949 的 AK / 整档下线退默认档但硬门仍拦）；MED-2 交叉校验（`{grant_id:tak1949-,account:""}` 抛错、反向抛错、一致放行、未知前缀不误判、端到端 revoke **不被置成 REVOKED**）；MED-3 分账号群（chat_id_for 优先级链 + _notify_internal_action/_notify_internal_failure/_alert_creds_undelivered 三处落各自群 + 发放失败端到端落 1949 群）；LOW-1 备注净化（伪造 `AccessKey Secret：` 行被压平、`AccessKey ID/Secret` 各恰一行、空白归一 5 参数化）；LOW-2 人名档精确相等（张三≠张三丰，撤销分支同样受约束）vs 企业名档保持宽松包含；1949 表单解析（无平台恒 aliyun、备注、by_name+by_id 双路、空 caps/缺桶拒、混入「平台=火山云」控件也不误读）；默认档解析零回归（不传 profile == 显式默认档、仍拒火山/unknown）；按 code 定档 + routes 白名单（含缺 code 不放行 / 缺 AK **仍放行**并在建号处显式失败 / 装配抛错不带崩基础 code）。
  **变异验证（8 处，证明断言非空）**：① permsync_client 退回"永远零参默认 client" → 默认 client 真去删了 `tempak-1949-sub-abc`（我的 sweep 断言 FAIL）；② by_grant_id 退回"认不出就当默认档" → 1949 记录真落进 `temp_ak:grant:`；③ make_ram_client 去掉显式分支 → ak 变 `LTAI_ENV_HIJACK`；④ classify_mode 退回不看账号 → 1949+6h 真判 sts；⑤ `_issue_sts` 去掉硬门 → 真拿默认账号 role ARN 调 AssumeRole；⑥ permsync_client 去掉交叉校验 → 矛盾 grant 真造出默认 client；⑦ note 不净化 → 凭证评论真出现**两行** `AccessKey Secret：`（`SK_REAL` + `FAKE_SECRET`，伪造可行）；⑧ 防串退回宽松包含 → 「张三丰」真能过「张三」的校验。
  **修既有 3 处（只动 tests/）**：① `test_temp_ak_naming.py::test_issue_ram_passes_display_name` 桩 `permsync_client` 由零参改收一个参数，并**断言收到的正是那个 grant**（签名变更的意义所在）；② `test_temp_ak_orchestrator.py` 加 autouse 钉 `TEMP_AK_STS_MAX_SECONDS=43200` —— 该文件 3 条「1h→sts」用例原先**依赖运行环境的 .env**，服务器 .env 是 0 → 同一份代码在容器里必红（已在容器用 `-e TEMP_AK_STS_MAX_SECONDS=43200` 反证：3 红转 20 passed），属测试卫生、**非 #63 引入但被 #63 的容器验收暴露**；③ `test_temp_ak_allowlist.py` 加 autouse 清空 1949 三项配置 —— #63 后白名单并入所有已注册档案的发放 code，该文件按「恰好三个 code」断言，跑在配了 1949 的环境（服务器/容器）会多出真实 code 而红（**这条是 #63 引入的环境耦合**）。三处修完容器只剩 2 条 matplotlib 基线。
  **未发现 dev 改动里的真问题**（隔离面严密）：grant 记录仍无 secret 键、1949 的 AK 不落 Redis、四条取凭证路径（issue/rewrite/revoke/sweep）全按 grant 分派、STS 越权口双保险堵死、矛盾 grant fail-closed 不产生"假成功 REVOKED"。**发现 1 个真 bug（MED，非隔离面，已 strict-xfail 锁 + 发 dev）**：`delivery._access_lines` 按 `oss-{region}` 拼 endpoint，而本仓库有**两种 region 书写约定** —— `permsync.BUCKET_MAP`（默认档 resolve_bucket 的回退表，硬编码在仓库里）是带前缀的 `oss-ap-southeast-1`，服务器 `TEMP_AK_1949_BUCKET_MAP` 是裸的 `cn-shenzhen`。已实测复现：`resolve_bucket('新加坡-wuji-sing')` → region `oss-ap-southeast-1` → 凭证评论写出 `外网 Endpoint：oss-oss-ap-southeast-1.aliyuncs.com` / `桶域名：wuji-sing.oss-oss-ap-southeast-1.aliyuncs.com`，把**错的连接信息**发给外部使用方 —— 正是这几行本想避免的「403 以为凭证无效」。1949 档按裸地域配置故当前不受影响（另有一条正向用例钉住裸地域拼对）。锁在 `test_access_lines_never_double_the_oss_prefix`（xfail strict，dev 修完会 XPASS 转红提示解除）。修法：归一约定或按 `oss-` 前缀判重。**E2E 归 dev**：真实飞书审批（1949 模板 0133C4FC…）走一单验建号落 1949 账号 + 凭证评论 + 到期清理，我这边只能到容器单测层。

[2026-07-29 TESTER] #63 二轮（dev 修完我报的 endpoint 双前缀 bug + 新增 resolve_bucket 反查）补测收口。**本机 .venv 全量 1758 passed / 1 skipped(h5py 既存) / 9 deselected(integration) / 0 failed / 0 xfailed**（跑 4 遍：默认序 ×3 + `-p no:randomly` ×1 全一致）；temp_ak 子集 468。较上轮 +19 例（新增 21、删 2：原 xfail + 它的临时伴生用例）。
  · **xfail 翻正**：`_access_lines` 现按 `region.startswith("oss-")` 归一 → 我报的 MED 已闭合，`test_access_lines_never_double_the_oss_prefix` 从 strict-xfail 拆成 4 组正常用例：裸 `cn-shenzhen`→`oss-cn-shenzhen.aliyuncs.com`（逐行 `==` 断言，含桶域名行）/ 已带前缀 `oss-ap-southeast-1`→**不再双写**（这条就是原 bug 的回归钉子）/ 空+空白+None region 三参数化→仍走「地域/Endpoint：未知」单行分支且不出现 `oss-.aliyuncs.com`、无桶域名行 / 无 bucket→只两行。再加 `resolve_bucket→_access_lines` 端到端 3 参数化（展示名、真实桶名反查、北京桶）+ `credential_text` 整篇层面钉一次（使用方真正拿到的那段文本无 `oss-oss-`）。
  · **resolve_bucket 按真实桶名反查（真机首单暴露）9 例**：①展示名正查不变（反查不干扰正查，两档各一条）②真实桶名反查拿到 region（1949 `wuji-product`→`cn-shenzhen`、默认档同理）+ 端到端确认正文不再是「未知」③都不中→`("", display)` 不抛 ④默认档反查 `permsync.BUCKET_MAP` 拿到**带前缀** region（`wuji-sing`/`wuji-test-data` 两桶）⑤**1949 档正查反查都不碰 `BUCKET_MAP`**（且同一输入默认档仍解析得到，证明不是表失效）⑥反查不成为跨账号通道（两档各自真实桶名互不可见）⑦坏 JSON 值（非 dict / 缺 bucket 键）反查不炸 ⑧落库层面填真实桶名→`region` 有值、`bucket_display` 记原文。
  · **allowlist 补正向用例 2 例**（dev 点出「只清配置会把功能测漏」，认同）：注册 1949 后白名单**应当**变成 4 个且含 `0133C4FC…`；并且该 code 真能被放行、`/feishu/event` 起的线程 target 恰是 `handle_temp_ak_event`（不是只进集合、事件却被丢弃）。
  · **变异验证 3 处（本轮新增，仍是非空断言）**：⑨ `_access_lines` 退回无归一 → `oss-oss-ap-southeast-1.aliyuncs.com`（新的逐行 `==` 断言 FAIL）；⑩ `resolve_bucket` 退回无反查 → `('','wuji-product')`，`_access_lines` 真退化成「地域/Endpoint：未知」单行（正是首单踩的现场）；⑪ 白名单退回不并入各档 code → 3 个且不含 1949（新断言 FAIL）。
  · **容器状态（重要）**：服务器仍是 `2808296`，**dev 这两处新修复尚未部署**（已核容器内 `delivery.py` 无 `startswith("oss-")`、`orchestrator.py` 无反查）。带新测试在容器跑 = **1743 passed / 13 failed**，其中 **11 条恰好就是钉这两处新修复的用例**（endpoint 归一 + resolve_bucket 反查两组，逐条对得上）、另 2 条是 matplotlib 基线 → 这 13 红本身就是「未部署」的铁证，不是回归。**dev 部署后请让我复跑容器收口**（上一轮同样口径下容器是 1734 passed / 2 failed matplotlib）。
  · dev 已补真机 E2E（我未覆盖的那块）：首单 grant `tak1949-de05e0a68c279e3f`、`account=1949`、`mode=ram`（STS fail-closed 生效）、RAM 用户 `tempak-1949-test-d03c38` **在 1949 账号存在 / 在默认账号 EntityNotExist**、Redis 落 `temp_ak_1949:`、grant 无 secret、策略三条结构+内嵌时间窗正确 —— 与单测的隔离结论真机对上。
  · 本轮**未发现新的源码问题**：两处改动的边界（空/空白 region、坏映射值、跨账号反查、带/不带前缀两套写法）都与断言吻合。

[2026-07-29 TESTER] #64（RAM 建号失败重复评论）补测完成 —— 该块此前**零覆盖**，现新增 `tests/unit/test_ram_approval_duplicate_notice.py`（53 例）。**本机 .venv 全量 1811 passed / 1 skipped(h5py 既存) / 9 deselected / 0 failed**（跑 4 遍：默认序 ×3 + `-p no:randomly` ×1 全一致）；ram_approval 子集 99。
  两道机制分层钉死：**终态标记**（`_is_terminal_error`+`_is_instance_done`+`save_approval_failure` 写 `error_terminal`）管「后续事件在入口短路、连云 API 都不再打」；**播报闸门**（`_claim_failure_notice`）管「同实例同错误只播报一次」，与是否落库无关、覆盖所有失败。
  · `_is_terminal_error`：四个 marker 各一条（含大小写混合 `InVaLiDpAsSwOrD`、中文「密码不符合」、`invalidloginname`）+ 真机形态（关键词埋在火山 ApiException 的长 HTTP dump 里，`(400) Reason: Bad Request` + header + `HTTP response body:{"Error":{"Code":"InvalidPassword",...}}`）；**收窄回归钉子**：`"(500) gateway timeout, malformed upstream response"`、`"InvalidParameter.Throttling"`、`"ParameterInvalid: transient backend error"`、`"(502) Bad Gateway: malformed response"` 一律判**可重试**，并直接断言 `_TERMINAL_ERROR_MARKERS` 集合恰为那四个、三个宽泛词不在其中（谁手滑加回去就红）；瞬时错误族（超时/连接重置/Throttling/503/空消息）全可重试。
  · `_is_instance_done`：success/dry_run→True（原行为不变）；failed+terminal→True；failed+`error_terminal=False`→False；**failed 但缺该键（线上已有的历史记录）→False 仍可自愈**；processing/None/""/0/空记录/空 code→False；只有 terminal 但状态非 failed 的脏数据→False。
  · `save_approval_failure`：密码策略错→`error_terminal=True` 且 `_is_instance_done` 串起来为 True；超时→False 且仍可自愈；`False` 确实落进 Redis 原文（`_save_instance_record` 会滤 `None`，显式钉住 False 不被当空值丢）。
  · `_claim_failure_notice`：同实例同错误第二/第三次→False；**换错误内容→True**（有信息量）且重复的仍挡；不同实例同文本→各报一次；键形状 `ram_approval:instance:failnotice:{inst}:{sig}` + TTL ∈ (0, 7天]；**Redis 不可用→恒 True 放行**（宁可重复也别漏报失败）。
  · **dev 对我 (b) 的推理成立、已钉死**：`notify_failure` 无条件先过闸门 → 不落库、直接连调三次同一 (instance, 错误) 只发**一条**评论（并断言确实 `load_approval_record == {}`）；瞬时失败重试三次也只一条；换错误发两条；无 instance_code 时闸门不介入也不占键、不走评论路径；评论发送抛异常不外泄。
  · **端到端（对着真机 BC9CE2EB 形态）**：同实例连推 3 个 APPROVED 事件、每次建号抛密码策略错 → ①评论恰 1 条 ②**云 API 恰调 1 次**（第 2、3 个事件在 `_is_instance_done` 短路，返回 `already_processed`）③记录 failed+terminal ④评论是人话「密码策略」不是原始 HTTP dump；连推 7 次（真机事件数）仍 1 次 1 条；**用户改密码后新开实例照常建号**（终态按实例、没误挡）。自愈面：超时失败→下个事件仍重试（云调用 2 次、评论仍 1 条）；超时→重试成功→落 success→后续短路；成功首次→后续 `already_processed` 且零评论；超时→再撞密码策略错→发第 2 条（不同错误）→此后终态短路。
  · **变异验证 5 处**：MUT-I 退回「_is_instance_done 只认 success/dry_run」→ 云调用 3 次（我的 `create_calls==1` FAIL）；MUT-J 只去播报闸门（终态标记还在）→ 该场景仍 1/1，说明**闸门在密码策略场景不承重**；MUT-M 去闸门 + 瞬时失败 → 评论 3 条（我的 `comments==1` FAIL）→ 证明**闸门是瞬时重试那层的承重件**，两道机制各管一层、都不可省；MUT-K 两道全退回 = **完整复现真机现场：3 次云调用 + 3 条重复评论**；MUT-L 把 `malformed`/`invalidparameter` 加回 markers → 两条瞬时错误立刻被判终态（自愈堵死），我的收窄断言 FAIL。
  · 本轮**未发现新的源码问题**。dev 采纳我的 (a) 收窄 marker 后，`_is_terminal_error` 只对确定性失败生效、判不准一律按可重试，方向正确。
  · **真机云侧遗��（dev 提供，无法单测复现，仅记账）**：① 火山 IAM 残留空壳用户 `hurunze`（无组无 AK —— 密码策略错发生在 `create_user` 之后、`CreateLoginProfile` 阶段）；② volcano `get_login_profile` 对无登录配置的用户返回**全零 stub 而非 NotExist** → 重提审批可能误判「已存在」而跳过创建，结果账号建成却没有控制台密码、还被判成功。**②比重复评论危险**，修它需先真机确认火山该 API 语义，不在本轮。
