...
class Config:
    ...
    _REQUIRED_FIELDS = [
        ...
        ("BOT_CREDS_ENCRYPTION_KEY", "用户 AK/SK 绑定不可用：缺 Fernet 密钥，加密写入会抛 CryptoNotConfigured"),
        ("ALIYUN_BOT_MASTER_AK_ID", "STS 多租户失效：缺 Master AK，用户云操作全部退回全局 AK 兜底（越权+审计归属错人）"),
        ("ALIYUN_BOT_MASTER_AK_SECRET", "STS 多租户失效：缺 Master AK Secret（同上）"),
        ("REDIS_HOST",             "会话记忆 / 配额管理不可用（降级到内存）"),
        ("REDIS_PASSWORD",         "Redis 无密码：对话历史/STS 临时凭证/审批记录均明文存在其中，任何能连到该端口的人可直接读取"),
        ("ADMIN_FEISHU_OPEN_ID",   "管理员门禁全部拒绝：OSS 权限下发、PFS 直传确认一律拒绝；跨云/SSH 迁移仅超阈值的确认被拒（阈值内不受影响）；GPU 工单不再自动批准，改为拒绝并评论提示"),
        ("RAM_QUERY_API_TOKEN",    "/api/ram/user 全量 403：已改为 fail-closed，不再回退共用 FEISHU_VERIFICATION_TOKEN（两个面共用一把钥匙 = 任一泄漏另一个全开）"),
        ("GPU_DIST_TOKEN",         "/gpu/distribution 全量 403 且摘要卡不再渲染入口按钮：同上，不再回退共用 webhook token"),
