...
def _probe_instance(name: str, gpu_count: int) -> dict:
    ...
    return {
        "name": name, "gpu_count": gpu_count,
        "gpu_util": gpu_util, "net_retran": net_ret,
        "temp": temp, "score": score, "level": level,
        "trend": trend, "avg30": avg30,
        "m": m,
    }
...
def cluster_health_report(top_n: int = 20) -> str:
    """
    拉取所有 Running GPU 实例，并行巡检，输出：
      - 全局概览：实例总数、空转数、总费用、网络告警数
      - 实例排行表（健康评分从低到高，需关注的排前面）
      - 优先操作列表
    """
    if not settings.PROMETHEUS_URL:
        return "❌ PROMETHEUS_URL 未配置，无法读取指标。"
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        for fut in as_completed(futs):
            try:
                ...
                inst = futs[fut]
                results.append({
                    "name": inst["name"], "gpu_count": inst.get("gpu_count", 1),
                    "gpu_util": None, "net_retran": 0, "temp": None,
                    "score": 50, "level": "⚪ 数据不可达",
                    "trend": "无数据", "avg30": None, "m": {},
                })
    ...
    idle_cost_est = idle_gpu_h * settings.GPU_PRICE_PER_HOUR  # 每小时浪费
